...
::: {.callout-important}
## Online calculator has been updated

The online calculator was updated in July 2025 and the underlying models modified. The online part of this package needs significant changes to support the new version of the online calculator.
:::
...
::: {.callout-important}
## SLE and APS no longer supported

The original online calculator was inconsistent for pregnancies with SLE and/or APS - it only increased risk when both were present while it should have been enough with only one of them. Since this is a rare, high-risk group that should be handled by specialists, we no longer have it in the calculator.
:::
...
```{r}
blandr.draw(df$mom_MAP.x , df$mom_MAP.y)
```

The systematic error seems to be isolated to cases with chronic hypertension:

```{r}
ggplot(df, aes((mom_MAP.x + mom_MAP.y) / 2, mom_MAP.y - mom_MAP.x, color = chronic_hypertension)) +
    geom_point() + 
  labs(x = "Mean", y = "Difference", caption = "MoM MAP") +
  theme(legend.position = "bottom")
```

And, looknig at only cases with chronic hypertension, cases with weight > 131 kg:

```{r}
ggplot(df[chronic_hypertension == "yes"], aes((mom_MAP.x + mom_MAP.y) / 2, mom_MAP.y - mom_MAP.x, color = weight > 131)) +
    geom_point() + 
  labs(x = "Mean", y = "Difference", caption = "MoM MAP") +
  theme(legend.position = "bottom")
```

Also note that the difference becomes even smaller if we round the formula-based values to three decimals:
...
Please note that while the difference is systematic, it is very small (about 0.001).

Bland Altman plot:

```{r}
blandr.draw(df$mom_PI.x , df$mom_PI.y)
```
...
```{r}
blandr.draw(df$mom_PlGF.x , df$mom_PlGF.y)
```

Some of the difference seems to be related to weight:

```{r}
ggplot(df, aes(weight, mom_PlGF.y - mom_PlGF.x)) +    geom_point() + labs(x = "Weight [kg]", y = "Difference", caption = "MoM PlGF")
```
...
Note that the largest difference is seen for those with previous PE:

```{r}
ggplot(df, aes(
    (nrisk_prior.x + nrisk_prior.y) / 2,
    nrisk_prior.y - nrisk_prior.x,
    color = previous_pe)) +
    geom_point() + scale_x_log10() +
    labs(x = "Mean", y = "Difference", caption = "Prior risk")
```
...
```{r}
blandr.draw(df$nrisk_prior.x , df$nrisk_prior.y)
```
...
```{r}
blandr.draw(df$nrisk.x , df$nrisk.y)
```
